import React from 'react';

const Hero = () => {
  return (
    <section className="relative min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden">
      {/* Main Content */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-32">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-8">
            {/* Main Heading */}
            <div className="space-y-4">
              <h1 className="text-5xl lg:text-6xl font-bold leading-tight">
                <span className="text-gray-800">Reimagine</span>
                <span className="text-purple-600">.</span>
                <span className="text-gray-400 font-light">Redefine</span>
                <span className="text-purple-600">.</span>
                <br />
                <span className="text-purple-600">Revolutionize</span>
                <span className="text-purple-600">.</span>
              </h1>
            </div>

            {/* Description */}
            <p className="text-lg text-gray-600 max-w-lg leading-relaxed">
              Transform your business into a brand with our web development, branding, social media 
              management, and product photography services—all under one roof !
            </p>

            {/* CTA Buttons */}
            <div className="flex flex-col sm:flex-row gap-4">
              <button className="bg-purple-600 text-white px-8 py-4 rounded-full font-semibold hover:bg-purple-700 transition-colors shadow-lg">
                Request a Quote
              </button>
              <button className="border-2 border-purple-600 text-purple-600 px-8 py-4 rounded-full font-semibold hover:bg-purple-50 transition-colors">
                Explore Our Services
              </button>
            </div>
          </div>

          {/* Services Visualization */}
          <div className="relative">
            <div className="relative w-full h-96">
              {/* Service Cards */}
              <div className="absolute top-0 right-0 transform rotate-12">
                <div className="bg-purple-600 text-white px-6 py-3 rounded-lg shadow-lg font-semibold">
                  BRANDING
                </div>
              </div>
              
              <div className="absolute top-16 right-20 transform rotate-12">
                <div className="bg-white text-gray-800 px-6 py-3 rounded-lg shadow-lg font-semibold border">
                  WEBSITE
                </div>
              </div>
              
              <div className="absolute top-32 right-40 transform rotate-12">
                <div className="bg-white text-gray-800 px-6 py-3 rounded-lg shadow-lg font-semibold border">
                  SOCIAL MEDIA
                </div>
              </div>
              
              <div className="absolute top-48 right-60 transform rotate-12">
                <div className="bg-white text-gray-800 px-6 py-3 rounded-lg shadow-lg font-semibold border">
                  PRODUCTSHOOT
                </div>
              </div>
              
              <div className="absolute bottom-0 right-80 transform rotate-12">
                <div className="bg-white text-gray-800 px-6 py-3 rounded-lg shadow-lg font-semibold border">
                  BRANDING
                </div>
              </div>

              {/* Decorative Elements */}
              <div className="absolute top-10 right-10 w-2 h-2 bg-red-500 rounded-full"></div>
              <div className="absolute top-24 right-32 w-2 h-2 bg-red-500 rounded-full"></div>
              <div className="absolute top-40 right-52 w-2 h-2 bg-red-500 rounded-full"></div>
              <div className="absolute bottom-20 right-72 w-2 h-2 bg-red-500 rounded-full"></div>
            </div>
          </div>
        </div>
      </div>

      {/* Floating WhatsApp Button */}
      <div className="fixed bottom-8 right-8 z-50">
        <button className="w-14 h-14 bg-green-500 rounded-full flex items-center justify-center shadow-lg hover:bg-green-600 transition-colors">
          <svg className="w-8 h-8 text-white" fill="currentColor" viewBox="0 0 24 24">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.885 3.488"/>
          </svg>
        </button>
      </div>
    </section>
  );
};

export default Hero;